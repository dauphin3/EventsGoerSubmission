...
//Add function for leave
pub fn handle_leave_event(event_address: HashString) -> ZomeApiResult<()> {
    hdk::remove_link(&AGENT_ADDRESS, &event_address, "member_of");//Remove link
    hdk::remove_link(&event_address, &AGENT_ADDRESS, "has_member"); //Both ways
    Ok(())
}
